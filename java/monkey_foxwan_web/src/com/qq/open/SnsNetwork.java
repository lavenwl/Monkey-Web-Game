...
public class SnsNetwork {
	...
	public static String postRequest(String url,
		...
		if (cookies != null && !cookies.isEmpty()) {
			...
			while (iter.hasNext()) {
				Map.Entry entry = (Map.Entry) iter.next();
				buffer.append((String) entry.getKey()).append("=").append(
						(String) entry.getValue()).append("; ");
			}
			// 设置cookie策略
		...
		try {
			try {
				...
				if (statusCode != HttpStatus.SC_OK) {
					throw new OpensnsException(ErrorCode.NETWORK_ERROR,
							"Request [" + url + "] failed:"
			...
			throw new OpensnsException(ErrorCode.NETWORK_ERROR, "Request ["
	 ...
	 *             网络故障时抛出异常。
	 */
	public static String postRequestWithFile(String url,
			HashMap<String, String> params, HashMap<String, String> cookies,
			FilePart fp, String protocol) throws OpensnsException {
		if (protocol.equalsIgnoreCase("https")) {
			Protocol httpsProtocol = new Protocol("https",
					new MySecureProtocolSocketFactory(), 443);
			Protocol.registerProtocol("https", httpsProtocol);
		...
		if (params != null && !params.isEmpty()) {
			...
			while (iter.hasNext()) {
				...
				postMethod.addPart(new StringPart((String) entry.getKey(),
						(String) entry.getValue(), CONTENT_CHARSET));
			}
		}
