...
public class SnsStat {
	 ...
	 *            上报参数
	 */
	public static void statReport(long startTime, String serverName,
			HashMap<String, String> params, String method, String protocol,
			int rc) {
		// // host => ip
		...
		try {
			...
			String sendStr = String
					.format(
							"{\"appid\":%s, \"pf\":\"%s\",\"rc\":%d,\"dst_ip\":\"%s\",\"protocol\":\"%s\",\"method\":\"%s\",\"time\":%.4f,\"timestamp\":%d,\"collect_point\":\"sdk-java-v3\"}",
							params.get("appid"), params.get("pf"), rc,
							InetAddress.getByName(serverName).getHostAddress(),
							protocol, method, timeCost, endTime / 1000);
			...
			DatagramPacket sendPacket = new DatagramPacket(sendBuf,
					sendBuf.length, addr, reportSvrport);
