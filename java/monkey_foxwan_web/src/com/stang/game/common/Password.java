...
public class Password {
	...
	public static String digestPassword(String password, String roleInfo) {
		try {
			...
			return encoder.encode(roleInfo.getBytes()) + "santang="
					+ encoder.encode(salt) + encoder.encode(digest);
	...
	public static byte[] byteConversionAscii(byte[] bytes) {
		...
		byte key = ch[new Random().nextInt(ch.length)];
		byte bt[] = new byte[bytes.length + 1];
		bt[0] = key;
		for (int i = 0; i < bytes.length; i++) {
			bt[i + 1] = (byte) (bytes[i] ^ key);
		}
