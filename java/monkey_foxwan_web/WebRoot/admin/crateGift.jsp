...
function checkAll(){
	...
	items= document.getElementById("listName");
...
if(valueNum!="" && valueNum>0){
  ...
  <% 
   ...
   %>
     ...
     <s:form action="addGiftBag.action" method="post" onsubmit="return checkAll()">
     <table>
     <tr>
     <td> 
     选择礼包类别:<select id="giftNameList" onchange="change()">
    <option>请先选择礼包类别</option>
    <%
    	for(EntityGiftBagTypeInfoDetail eg:egbtid){
    	%>
    		<option value="<%=eg.getID() %>"><%=eg.getGTI_NAME() %></option>
    	<%
    	}
     %>
    
    </select></td>
    <td>礼包名字：<input type="text" name="giftBagName" id="giftBagName" /></td>
    <td>开启等级：<input type="text" name="openLive" id="openLive" /></td>
    </tr>
    <!-- <tr>
    <td>起始时间：<input type="text" name="stime1" id="stime1" onclick="MyCalendar.SetDate(this)" readonly="readonly" /></td>
	<td colspan="2">结束时间：<input type="text" name="stime2" id="stime2" onclick="MyCalendar.SetDate(this)" readonly="readonly" /></td>
	</tr>   -->
	<tr>
    <td colspan="3">礼包描述：<textarea type="text" name="item_prop" id="item_prop" style="width: 500px;height: 300px" ></textarea></td>
    </tr>
    </table>
    	...
    	<table border=0>
				...
				<tr
					...
					<!-- <td>
						<input type="checkbox"
							onCLick="functionGetGift1('<%=equip.getEquipName()%>','2-1-<%=equip.getId()%>')" name="checkList" /><%=equip.getEquipName()%></td>  -->
							<td>
						<input type="checkbox"
				...
				<tr
					...
					<td>
						<!-- <input type="checkbox"
							onCLick="functionGetGift('<%=avatar.getAvatarName()%>','4-1-<%=avatar.getId()%>')" /><%=avatar.getAvatarName()%> -->
							<input type="checkbox"
				...
				<tr
					...
					<td>
						<!-- <input style="width: 15px"
							onblur='functionGetGiftItem("<%=item.getItemName()%>","<%=item.getId()%>",value)' /><%=item.getItemName()%> -->
							<input style="width: 15px"
							onblur='functionGetGiftItem1("<%=item.getId()%>",value,"<%=item.getItemName()%>")' /><%=item.getItemName()%>
							</td>
					<%
								if (++i % 6 == 0) {
								j++;
